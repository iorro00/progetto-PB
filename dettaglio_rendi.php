    ...
    if ($projectId) {
        ...
        foreach($project as $value) {
            ...
            foreach($risInt as $rsi){
                ...
                $tabRisorseInt.="</td><td>".$ruolo."<td><td class='get-id' data-id='".$rsi["id"]."'><span class='insOreEff'>+</span></td></tr>";
            }
            ...
            foreach($risExt as $rse){
                $tabRisorseExt .= "<tr><td>".$rse["nominativo"]."</td><td>".$rse["oreDocenza"]." (".$rse["oreDocenzaEffettive"].")</td><td>".$rse["costoPrevisto"]."(".$rse["costoEffettivo"].")</td><td>".$rse["costiEventuali"]."</td><td class='get-id-ext' data-id-ext='".$rse["id"]."'><span class='insOreDocEff'>+</span></td></tr>";
            }
        }
...
// Apri popup per le risorse interne
document.querySelectorAll('.get-id').forEach(span => {
    span.addEventListener('click', function() {
        const dataId = this.getAttribute('data-id');
        const element = document.getElementById('get-id2');
        const projectId = element.dataset.id;
        const mess2 = "int";
        // Mostra loader e carica dati
        document.getElementById("contenuto").innerHTML = '<div style="text-align:center;padding: 30px 0;"><span class="spinner-border text-primary" role="status"></span></div>';
        jQuery.ajax({
            type: 'POST',
            ...
            success: function(response) {
                if (response.success) {
                    document.getElementById("contenuto").innerHTML = response.risposta;
                    popup.style.display="flex";
                    disableScroll();
                } else {
                    document.getElementById("contenuto").innerHTML = "<p class='text-danger'>Errore nel caricamento dati</p>";
                }
            }
        });
    });
